...
export function StoreProvider({children,initial}:{children:ReactNode;initial:StoreData}){
 ...
 const [userLoading,setUserLoading]=useState(initial.configured),[userError,setUserError]=useState('');
 const sessionRequest=useRef<Promise<StoreUser|null>|null>(null);
 const refreshUser=useCallback(()=>{
  if(sessionRequest.current)return sessionRequest.current;
  if(!initial.configured){setUser(null);setUserLoading(false);return Promise.resolve(null)}
  setUserLoading(true);setUserError('');
  // Share an in-flight refresh so page entry and provider startup do not rotate
  // the same refresh token concurrently.
  const request=api('me').then(d=>{const current:StoreUser|null=d.user??null;setUser(current);return current}).catch(e=>{setUser(null);setUserError(e instanceof Error?e.message:'Unable to check your session. Please try again.');return null}).finally(()=>{sessionRequest.current=null;setUserLoading(false)});
  sessionRequest.current=request;return request;
 },[initial.configured]);
