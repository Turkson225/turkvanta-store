...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}){const {slug}=await params;const data=await getStoreData();const p=data.products.find(p=>p.slug===slug);return {title:p?`${p.name} | Turkvanta`:'Product unavailable | Turkvanta',description:p?.description,alternates:{canonical:'/product/'+slug}}}
